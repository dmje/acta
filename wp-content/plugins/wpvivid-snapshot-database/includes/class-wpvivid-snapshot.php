...
if (!defined('WPVIVID_SNAPSHOT_PLUGIN_DIR'))
{
    die;
}

class WPvivid_Snapshot
{
    public $options;
    public $main_tab;
    ...
    public function __construct()
    {
        if(is_admin())
        {
            include_once WPVIVID_SNAPSHOT_PLUGIN_DIR . 'includes/class-wpvivid-snapshot-function.php';
            include_once WPVIVID_SNAPSHOT_PLUGIN_DIR . 'includes/class-wpvivid-snapshot-options.php';
            include_once WPVIVID_SNAPSHOT_PLUGIN_DIR . 'includes/class-wpvivid-snapshots-list.php';
            ...
            $this->options=new WPvivid_Snapshot_Option();
            if (is_multisite())
            {
    ...
    public function get_toolbar_menus($toolbar_menus)
    {
        ...
        $display = apply_filters('wpvivid_get_menu_capability_addon', 'menu_backup');
        if($display) {
            $menu['id'] = 'wpvivid_admin_menu_snapshot';
            $menu['parent'] = 'wpvivid_admin_menu';
            $menu['title'] = 'Database Snapshots';
            $menu['tab'] = 'admin.php?page=' . apply_filters('wpvivid_white_label_plugin_name', 'wpvivid-snapshot');
            $menu['href'] = $admin_url . 'admin.php?page=' . apply_filters('wpvivid_white_label_plugin_name', 'wpvivid').'-snapshot';

            $menu['capability'] = apply_filters("wpvivid_menu_capability","administrator","wpvivid-snapshot");
    ...
    {
        $screen_ids=array();
        $screen_ids=apply_filters('wpvivid_snapshot_get_screen_ids',$screen_ids);
        if(in_array(get_current_screen()->id,$screen_ids))
        {
            wp_enqueue_style('wpvivid_snapshot', WPVIVID_SNAPSHOT_PLUGIN_URL . 'includes/display/css/wpvivid-snapshot-style.css', array(), WPVIVID_SNAPSHOT_VERSION, 'all');
        }
    }
    ...
    public function enqueue_scripts()
    {
        ...
        if(in_array(get_current_screen()->id,$screen_ids))
        {
            wp_enqueue_script('wpvivid_snapshot_js', WPVIVID_SNAPSHOT_PLUGIN_URL . 'includes/display/js/wpvivid-snaphost.js', array('jquery','jquery-ui-dialog'), WPVIVID_SNAPSHOT_VERSION, false);
            wp_localize_script('wpvivid_snapshot_js', 'wpvivid_snapshot_ajax_object', array('ajax_url' => admin_url('admin-ajax.php'),'ajax_nonce'=>wp_create_nonce('wpvivid_ajax')));
        }
        else
        {
            wp_enqueue_style('wpvivid_quick_snapshot', WPVIVID_SNAPSHOT_PLUGIN_URL . 'includes/display/css/wpvivid-quick-snapshot-style.css', array(), WPVIVID_SNAPSHOT_VERSION, 'all');
        }
        wp_enqueue_style (  'wp-jquery-ui-dialog');
        wp_enqueue_script( 'jquery-ui-dialog' );
        wp_enqueue_script (  'wpvivid_qucick_snapshot_js' ,       // handle
            WPVIVID_SNAPSHOT_PLUGIN_URL . 'includes/display/js/wpvivid-quick-snapshot.js'  ,       // source
            array('jquery-ui-dialog'),
            WPVIVID_SNAPSHOT_VERSION, false
        );
        wp_localize_script('wpvivid_qucick_snapshot_js', 'wpvivid_quick_snapshot_ajax_object', array('ajax_url' => admin_url('admin-ajax.php'),'ajax_nonce'=>wp_create_nonce('wpvivid_ajax')));
    }
                            ...
                            <div class="wpvivid-backup">
                                ...
                                <div class="wpvivid-canvas wpvivid-clear-float">
                                    ...
                                    if(!class_exists('WPvivid_Tab_Page_Container_Ex'))
                                        include_once WPVIVID_SNAPSHOT_PLUGIN_DIR . 'includes/class-wpvivid-tab-page-container-ex.php';
    ...
    {
        if(!class_exists('WPvivid_Snapshot'))
        {
            include_once WPVIVID_SNAPSHOT_PLUGIN_DIR . 'includes/class-wpvivid-snapshot-function.php';
        }

        $snapshot=new WPvivid_Snapshot_Function();
        ...
        <div class="wpvivid-one-coloum wpvivid-workflow wpvivid-clear-float" style="margin-bottom:1rem;">
            ...
            <div id="wpvivid_snapshots_list">
                <?php
                $Snapshots_list = new WPvivid_Snapshots_List();
